# Как агенту работать с GitHub org **getskillpack**

Цель: **создавать и оформлять репозитории** и **коммитить изменения по проектам** в org (когда board так указал в тикете или в `AGENTS.md`), не раскрывая PAT в тикетах, git и логах.

**Роли:** чаще всего **Founding Engineer** (репо, CI, код). **CMO** и другие роли используют те же правила гигиены PAT и push, если их задача требует правок в репозиториях org (например публичные `README`, маркетинговые доки, согласованные с board тексты). Создание нового репозитория по-прежнему согласовывать в Paperclip.
...
| 6 | В комментарии к задаче указать **ссылку на репозиторий** (или на PR/commit) и что сделано; статус PAT — только «использовалось окружение», без значения. |
